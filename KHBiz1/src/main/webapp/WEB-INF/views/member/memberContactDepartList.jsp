...
<script type="text/javascript">
	$(document).ready(function() {
		$("#departmentFind").change(function() {
			 ...
			 $.ajax({
					url : "./memberContactDepartmentList",data :{
		...
		$('.main-panel').scroll(function () {
			if($('.main-panel').scrollTop()>($('#body').height()-$(window).height()+170)){
				var max = ${memberContactDepartList.size()};
				var department = '${memberContactDepartList[0].department}';
				if(max<20){	
				}else{
					setTimeout(function(){
						$.ajax({
							url:"memberContactDepartmentAjaxList",
							type:"get",
							data:{
								perPage:max*1+20,
								department:department
...
<table class="table">
	...
	<c:forEach items="${memberContactDepartList}" var="member" varStatus="i">
